...
func getApplicants() ([]*applicant, error) {
	...
	for scanner.Scan() {
		...
		a.LastName = fields[1]
		a.Departments = fields[7:]

		scores, err := calculateScores(fields[2:7])
		if err != nil {
			return nil, err
		}
		a.Scores = scores
...
func calculateScores(rawScores []string) (map[string]float64, error) {
	...
	scores["Physics"] = math.Max((scoreList[0]+scoreList[2])/2, scoreList[4])
	scores["Engineering"] = math.Max((scoreList[2]+scoreList[3])/2, scoreList[4])
	scores["Biotech"] = math.Max((scoreList[1]+scoreList[0])/2, scoreList[4])
	scores["Chemistry"] = math.Max(scoreList[1], scoreList[4])
	scores["Mathematics"] = math.Max(scoreList[2], scoreList[4])
